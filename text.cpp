...
void innerLoop(PAGE &page, int &beg, int &end, int &y, int &x, int &i,
    ...
    case KEY_F(6):
        ...
        break;
    case KEY_F(7):
        prompt_string("Open:", page.filename, NAME_LIMIT);
        if (file_exists(page.filename)) {
            filename = page.filename;
            load_file(&page, page.filename);
            print_page(&page, beg, end);
            update_status("File opened");
        } else {
            update_status("File does not exist");
        }
        break;
    case KEY_F(8):
        if (prompt_yesno("Are you sure you want to rebuild?")) {
            if(!extExecRepl("#eval " + filename)) {
                update_status("Rebuild failed");
            } else {
                update_status("Rebuild successful");
            }

            redrawwin(stdscr); // Redraw fix
            return;
        }
        break;
    case KEY_UP:
    ...
    case KEY_BACKSPACE:
        if (strlen(page.text[y + y_offset].line) ==
            0) { // can only delete blank lines for now
...
int textmain(int argc, char *argv[]) {
    PAGE page;
    ...
    while (running) {
        innerLoop(page, beg, end, y, x, i, running);
    }
